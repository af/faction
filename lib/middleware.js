            ...
            return function(action) {
                ...
                action.payload.then(function(result) {
                    ...
                    if (typeof _successCb === 'function') {
                        ...
                        var multi = Array.isArray(followUpAction)
                        ;(multi ? followUpAction : [followUpAction]).forEach(function(action) {
                            verifyAction(action, 'onSuccess cb return value')
                            dispatch(action)
                        })
                    }
                    ...
                    if (typeof _errorCb === 'function') {
                        ...
                        var multi = Array.isArray(followUpAction)
                        ;(multi ? followUpAction : [followUpAction]).forEach(function(action) {
                            verifyAction(action, 'onError cb return value')
                            dispatch(action)
                        })
                    }
                })
